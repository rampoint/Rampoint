...
firebase.auth().onAuthStateChanged((user) => {
  if (user) {
    document.getElementById("error-cupom").style.display = "block";
...
function makeCode(pontos, uid) {
  ...
  if (pontosQrcode > pontos) {
    ...
    document.getElementById(
      "error-cupom"
    ).innerHTML = `Você não possui a quantidade suficiente para pegar esse cupom:${pontosQrcode}`;
  }
...
function salvarCupom() {
  ...
  const pontosQrcode = cupomAtivo.getAttribute("value");
  const cupomText = document.querySelector(
    ".grupo-cupons .select-cupom.active .titulo-cupom"
  );
  ...
  firebase.auth().onAuthStateChanged((user) => {
    if (user) {
      ...
      firebase
        .database()
        .ref("users/" + globalUserId + "/cupons/" + codigoAleatorio)
        .set({

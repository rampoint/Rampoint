  ...
  function login() {
    ...
    db.collection('users').where('email','==',email).get().then((querySnapshot) => {
        querySnapshot.forEach((doc) => {
            console.log('acho o email mas nao acho a senha')
            db.collection('users').where('senha','==',senha).get().then((querySnapshot) => {
                querySnapshot.forEach((doc) => {
                    console.log(doc.data())
                    console.log('acho o email e acho a senha')
                    window.location.href = '../dashboard-adm/dashboard.html'
                })
            })
        })
    }).catch((error)=>{
        console.log('acho nao')
    })
  }
